...
(export 'linear-fit)
(defun linear-fit (&key (data (slot-value *table* 'list)) save)
  (multiple-value-bind (slope c r) 
      (cl-mathstats:linear-regression-brief (mapcar #'second  data)
					    (mapcar #'first data))
    (when save 
      (setf (symbol-function save) (lambda (x) 
				     (+ c (* x slope)))))
...
(export 'fit)
(defun fit (lambda initial &key (data (slot-value *table* 'list)) save-to)
  (let ((result (multiple-value-list (fit::fit lambda data initial t t))))
    (when save-to 
      (setf (symbol-function save-to) (save-fit-func lambda (second result))))
    (apply #'values result)))
  

(export 'multilinear-fit)
(defun multilinear-fit (&key (data (slot-value *table* 'list)) save)
  (multiple-value-bind (coeff sse dof)
      (cl-mathstats:multiple-linear-regression-brief (mapcar #'second  data)
						     (mapcar #'first (mapcar #'first data))
						     (mapcar #'second (mapcar #'first data)))
    
    ;; (when save 
    ;;   (setf (symbol-function save) (lambda (x) 
    ;; 				     (+ c (* x slope)))))
    (values (list (list "Coeff" coeff)
		  (list "SSE" sse)
		  (list "dof" dof))
	    coeff sse dof)))
...
(export 'fit-plot)
(defun fit-plot (fit-func x-axis file &optional (data *bigdata*) (xlabel "X-axis") (ylabel "Tp"))
  (let ((x-tp-fit (sort (loop 
			  for (x tp) in data 
			  collect (list (funcall x-axis x) tp (funcall fit-func x)))
			#'< :key #'first))
	(type (cond
		((uiop:string-suffix-p file ".png") :png)
		((uiop:string-suffix-p file ".svg") :svg)
		(t (error "unknown file type")))))
    (with-plot file
      (setup :xlabel xlabel :ylabel ylabel
	     :terminal (list type :size :|1500,1100| :font "Times New Roman, 30pt"))
...
(export 'loglog-fit)
(defun loglog-fit (&key (data (slot-value *table* 'list)) save)
  (multiple-value-bind (slope c r) 
